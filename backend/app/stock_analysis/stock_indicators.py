...
# Helper function to standardize window size handling across all indicator functions
def _get_window_size(data, window, default_window):
    """
    Helper function to determine window size from parameters or dynamically.
    
    Parameters:
        data (DataFrame): The stock price data.
        window (int, optional): Custom window size if provided.
        default_window (int): Default window size if no custom window is provided.
        
    Returns:
        int: The window size to use.
    """
    return window if window is not None else determine_window_size(data, default_window)
...
def calculate_SMA(data, ticker, window=None, default_window=20):
    ...
    """
    try:
        window_size = _get_window_size(data, window, default_window)
...
def calculate_VWAP(data, ticker):
    ...
    Returns:
        go.Scatter: A Plotly scatter trace for VWAP.
    """
    try:
        temp_data = data.copy()
...
def calculate_ichimoku_cloud(data, ticker, conversion_period=None, base_period=None, lagging_span_b_period=None,
    ...
    try:
        ...
        # Conversion Line (Tenkan-sen) - average of highest high and lowest low over conversion period
        conversion_line = (data['High'].rolling(window=conv_period).max() +
                          data['Low'].rolling(window=conv_period).min()) / 2
        
        # Base Line (Kijun-sen) - average of highest high and lowest low over base period 
        base_line = (data['High'].rolling(window=base_p).max() +
                    data['Low'].rolling(window=base_p).min()) / 2
        
        # Leading Span A (Senkou Span A) - average of conversion and base lines, shifted forward
        leading_span_a = ((conversion_line + base_line) / 2).shift(base_p)
        
        # Leading Span B (Senkou Span B) - average of highest high and lowest low over lagging period, shifted forward
...
# Helper function to extract indicator parameters from different configuration formats
def _extract_indicator_params(indicator_config):
    ...
    Returns:
        tuple: (indicator_name, params_dict)
    """
    indicator_name = None
    params = {}
...
# Helper function to add a trace to the chart in the correct panel
...
def add_indicator_to_chart(fig, data, indicator_config, ticker, panel_idx=1):
    ...
    try:
        ...
        # Extract indicator name and parameters using helper function
        indicator_name, params = _extract_indicator_params(indicator_config)
        ...
        if isinstance(result, tuple):
            ...
            return any(trace is not None for trace in result)
        else:
